...
lambda_vec = np.array([0.001, 0.01, 0.05, 0.1, 0.5, 1])
eta_vec = np.array([0.0001, 0.001, 0.01, 0.05, 0.1])
r_vec = np.array([1, 5, 10, 15, 20, 50, 80, 100])
...
for ix, row in tqdm(results.iterrows()):
    ...
    results.to_csv('Results/results_sgd_all.csv', index=False)
