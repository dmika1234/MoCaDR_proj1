...
def ParseArguments():
    ...
    parser.add_argument('--alg', required=True, help='We compute results from 1 to max_r dimensions')
...
results = pd.DataFrame({'r': r_vec, 'rmse_0': np.zeros(max_r), 'rmse_global_mean': np.zeros(max_r),
                             'rmse_row_means': np.zeros(max_r), 'rmse_colmeans': np.zeros(max_r), 'rmse_means_weighted': np.zeros(max_r)})
...
train_col_means = fillna_col_means(train_df)
train_means_weighted = fillna_means_weighted(train_df, 0.5)
...
if (func_name == 'perform_svd1') or (func_name == 'perform_nmf'):
    for ix, r in enumerate(tqdm(r_vec)):
        ...
        results.loc[ix, 'rmse_colmeans'] = func_to_call(train_col_means, test_array, r=r)
...
if func_name == 'perform_svd2':
    for ix, r in enumerate(tqdm(r_vec)):
        ...
        results.loc[ix, 'rmse_col_means'] = func_to_call(train_df.isna(),
                                                         train_col_means, test_array, r=r)[0]
