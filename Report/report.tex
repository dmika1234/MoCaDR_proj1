...
\tami{Similarly, probably since filling data with global mean doesn't differentiate the users or the movies}
...
We make this correction, beacause \tami{...}
...
So we decided to take $\vaepsilon = 0.0086$ in this method.
...
\subsection*{SGD}
In case of this method we have to set parameters $r$, $\eta$ and $\lambda$, but also set some stop condition.
...
Then we performed optimization with respect to $r$, $\eta$ and $\lambda$.
The result are as follows \tami{...}
